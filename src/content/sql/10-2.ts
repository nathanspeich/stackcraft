...
import { hasObject, sqlHas, steps } from '../checks'
...
-- SEARCH orders USING INDEX idx_orders_customer (customer_id=?)`,
  ...
  task: {
    kind: 'sql',
    instructions: '1. Run EXPLAIN QUERY PLAN on SELECT * FROM orders WHERE customer_id = 1 and read the detail column: it should say SCAN.\n2. Create an index named idx_orders_customer on orders(customer_id).\n3. Run the same EXPLAIN QUERY PLAN again and confirm it now says SEARCH ... USING INDEX.\n4. Create a second index idx_items_product on order_items(product_id), since product_id is a foreign key that joins use.',
    starter: '-- Measure, index, measure again\n',
    setup: SHOP,
    ...
    solution: { file: 'EXPLAIN QUERY PLAN SELECT * FROM orders WHERE customer_id = 1;\nCREATE INDEX idx_orders_customer ON orders(customer_id);\nEXPLAIN QUERY PLAN SELECT * FROM orders WHERE customer_id = 1;\nCREATE INDEX idx_items_product ON order_items(product_id);\n' },
    check: (r) => {
      const details = (r.results ?? []).flatMap((s) => s.values.map((v) => String(v[v.length - 1])))
      return steps([
        [!r.error, `Your script stopped with an error: ${r.error}`],
        [sqlHas(r, /EXPLAIN\s+QUERY\s+PLAN/) && details.some((d) => /^SCAN orders/.test(d)), 'Run EXPLAIN QUERY PLAN on the customer query before creating the index. The detail should read SCAN orders.'],
        [hasObject(r, 'idx_orders_customer') && /orders\s*\(\s*customer_id\s*\)/i.test(r.schema?.idx_orders_customer ?? ''), 'Create the index: CREATE INDEX idx_orders_customer ON orders(customer_id).'],
        [details.some((d) => /SEARCH orders USING INDEX idx_orders_customer/.test(d)), 'Run the same EXPLAIN QUERY PLAN after the index. It should say SEARCH orders USING INDEX idx_orders_customer.'],
        [hasObject(r, 'idx_items_product') && /order_items\s*\(\s*product_id\s*\)/i.test(r.schema?.idx_items_product ?? ''), 'Create idx_items_product on order_items(product_id).'],
